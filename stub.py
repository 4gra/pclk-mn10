...
def xprint(dat, pre="", asc=None):
    ...
    if asc or (asc == None and print_ascii == True):
        print("TX|%s" % " ".join(
            ["%2s" % chr(x) if (x > 31 and x < 127) else '..' for x in dat]))
...
def jread(ll, delay=None, asc=None, rtn=False):
...
# let's test a daemon reader
import threading
class LReader(threading.Thread):
    ...
    def run(self):
    	try:
    		while True:
	    		out = jread(*self.params)
	    		for msg in [o for o in out if len(o)]:
	    			length=msg[0]
	    			typ=msg[4]
...
if __name__ == '__main__':
    ...
    daemon = False
    # -- clumsy arg parsing
    if 'daemon' in argv[1:2]:
        print("Daemon mode, ^C / ctrl-brk to quit after commands issued...", file=sys.stderr)
        asc = ('--noascii' not in argv[1:])
        daemon = LReader(32, 1.0, asc, True)
        daemon.daemon = True
        daemon.start()
        argv = argv[1:]
    ...
    if "off" in argv[1:2]:
        ...
        for arg in argv[2:]:
            for x in hexin(arg):
                ...
                if not daemon:
                    jread(32, 0.2)
    elif (argv[0][-2:] == 'poll' or "poll" in argv[1:]) and not daemon:
        asc = ('--noascii' not in argv[1:])
        while True:
            jread(32, 0.1, asc)
